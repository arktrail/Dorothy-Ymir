...
def train_problem(current_depth, upper_depth, class_num, fine_tuning, embedding_weight, input_data, model_type, learning_categories):
    params = {"gpu": 0,
              ...
              "epoch": 10,  # TODO: change in the future
              "batchsize": 64,
              "unit": 1024,
    ...
    if (current_depth == "1st") and ((params["model_type"] == "CNN-fine-tuning") or (params["model_type"] == "CNN-Hierarchy")):
        network_output = cnn_train_mulgpu.load_top_level_weights(params)
    else:
        network_output = cnn_train_mulgpu.main(params)
...
def make_labels_hie_list_dic(labels, label_hierarchical_info_dic):
    ...
    for label in labels:
        layer_category_list_dic[int(
            label_hierarchical_info_dic[label])].add(label)
...
def main():
    ...
    input_data_dic = data_helper.data_load(
        train_lines, valid_lines, test_lines, category_hie_info_dic, use_words, label_name, input_text_name)
    category_hie_list_dic = make_labels_hie_list_dic(
        list(input_data_dic["catgy"].keys()), category_hie_info_dic)
    ...
    embedding_weight = data_helper.embedding_weights_load(
        input_data_dic["vocab"], embedding_weight_path)
    ...
    if categorization_type == "pre-process":
        ...
        x_trn, y_trn, x_val, y_val, x_tst, y_tst = data_helper.build_problem(
            learning_categories=learning_categories, depth=depth, input_data_dic=input_data_dic)
        input_network_data = {"x_trn": x_trn, "y_trn": y_trn,
                              "x_val": x_val, "y_val": y_val, "x_tst": x_tst, "y_tst": y_tst}
        y_pred = train_problem(current_depth=depth, upper_depth=upper_depth, class_num=len(learning_categories), fine_tuning=fine_tuning,
                               embedding_weight=embedding_weight, input_data=input_network_data, model_type=model_type, learning_categories=learning_categories)
    ...
    # ========================================================
        ...
        # this order is network"s output order.
        learning_categories = sorted(input_data_dic["catgy"].keys())
        x_trn, y_trn, x_val, y_val, x_tst, y_tst = data_helper.build_problem(
            learning_categories=learning_categories, depth="flat", input_data_dic=input_data_dic)
        input_network_data = {"x_trn": x_trn, "y_trn": y_trn,
                              "x_val": x_val, "y_val": y_val, "x_tst": x_tst, "y_tst": y_tst}
        y_pred = train_problem(current_depth="flat", upper_depth=None, class_num=len(learning_categories), fine_tuning=fine_tuning,
                               embedding_weight=embedding_weight, input_data=input_network_data, model_type=model_type, learning_categories=learning_categories)
        grand_labels, pred_result = data_helper.get_catgy_mapping(
            learning_categories, y_tst, y_pred, "flat")
        data_helper.write_out_prediction(
            grand_labels, pred_result, input_data_dic)
    ...
    # ========================================================
        ...
        if not os.path.exists("./CNN/PARAMS/parameters_for_multi_label_model_1st.npz"):
            raise FileNotFoundError(
                'Please change _tModelType=CNN-Hierarchy" or _tModelType=CNN-fine-tuning" to _tModelType=Pre-process" in example.sh.')
        print("-"*50)
        ...
        for layer in layers:
            ...
            if layer < MAX_LAYER:
                continue
            grand_labels, pred_result = data_helper.get_catgy_mapping(
                learning_categories, y_tst, y_pred, depth)
            upper_depth = depth
        ...
        print("Final Result")
        data_helper.write_out_prediction(
            y_tst_concat, y_pred_concat, input_data_dic)
